...
with (p/'rf-post-accounts-focused-first.log').open('w') as log:
 r=subprocess.run(['bash',str(p/'run-rf-post-accounts-focused.sh')],env=env,stdout=log,stderr=subprocess.STDOUT)
