...
paths=[Path('supabase/contract-migrations')/n for n in ['20260914090244_annual_accounts_filing_expand.sql','20260914092018_annual_accounts_filing_read_contracts.sql','20260914092924_annual_accounts_filing_preparation_contracts.sql','20260914093204_annual_accounts_filing_import_contract.sql']]
report={'database':c['dbname'],'mode':'ROLLBACK_ONLY_SYNTHETIC_CUTOVER_PHASE','artifacts':{str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in paths},'cases':[]}
with psycopg.connect(dsn,autocommit=True) as db:
 ...
 try:
  ...
  def call(name,actor,query,params,expected=None,fresh=True,aal='aal2',role='annual_accounts_filing_workflow_executor'):
   ...
   try:
    claims={'sub':ACTORS[actor],'role':'authenticated','aal':aal,'amr':[{'method':'totp','timestamp':int(time.time()) if fresh else 1}]}
  ...
  db.execute('set local role annual_accounts_filing_store_owner');db.execute("update backend_system.annual_accounts_migration_state set phase='cutover'");db.execute('reset role')
  for actor in ACTORS:
...
(p/'read-controls-database-results.json').write_text(json.dumps(report,ensure_ascii=False,indent=2,default=str)+'\n');print(json.dumps({k:v for k,v in report.items() if k!='cases'}));print('cases:',len(report['cases']))
