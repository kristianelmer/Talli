...
p=Path(__file__).parent;snapshot=p/'spec-physical-sql-wip';sys.path.insert(0,str(snapshot/'apps/backend/tests'))
...
with psycopg.connect(dsn,autocommit=True) as db:
 ...
 except Exception as e:report.update(status='PROBE_ERROR',error=str(e),sqlstate=getattr(e,'sqlstate',None))
 finally:
...
(p/'physical-sql-spec-target-rls-results.json').write_text(json.dumps(report,indent=2,default=str)+'\n');print(json.dumps(report,indent=2));assert report['status'].startswith('REPRODUCED');assert all(report[k]for k in ['schemaRollbackVerified','fixtureRollbackVerified','globalMembershipsRestored','sourceSchemaAclRestored'])
