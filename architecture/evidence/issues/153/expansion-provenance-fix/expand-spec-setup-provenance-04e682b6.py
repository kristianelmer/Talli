...
p=Path(__file__).parent;pin=p/'spec-expand-04e682b6';sys.path.insert(0,str(pin/'apps/backend/tests'))
from accounts_database_fixtures import seed,insert,FAMILIES,COMPANY,ACTORS,IDS
conn=json.loads((p/'expand-spec-clone-private-04e682b6.json').read_text())['DB_URL'];c=conninfo_to_dict(conn);assert c['host'] in ('127.0.0.1','localhost') and c['port']=='52966' and c['dbname']=='accounts153_spec_04e682b6_d060'
path=pin/'supabase/contract-migrations/20260914090244_annual_accounts_filing_expand.sql';raw=path.read_text();assert hashlib.sha256(raw.encode()).hexdigest()=='ea3c0601ec92539a5c9d672b5254a7342e6e63e4c4147e0ed7e6abec74ea7caa';script=raw.replace('begin;\n','',1)[:-len('commit;\n')]
report={'revision':'04e682b69e5426408932233d05328134e8e997f8','migrationSha256':hashlib.sha256(raw.encode()).hexdigest(),'database':c['dbname'],'mode':'ROLLBACK_ONLY'}
with psycopg.connect(conn,autocommit=True) as db:
 ...
 try:
  ...
  report['status']='CONFIRMED_SETUP_SCOPE_CLASSIFICATION_GAP'
...
(p/'expand-spec-setup-provenance-results-04e682b6.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print(json.dumps(report,ensure_ascii=False))
