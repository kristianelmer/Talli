...
export async function loadRf1086ProductionState(input: {
  ...
  if (
    ...
    typeof input.workspaceClient.from !== "function" ||
    !input.controlClient ||
    typeof input.controlClient.from !== "function" ||
  ...
  const [
    ...
    loadQuery<any>(input.workspaceClient.from("companies").select("id, org_number, name, entity_type, address, postal_code, city, status_text, source, created_by, identity_confirmed_at, identity_locked_at, created_at").eq("id", companyId).maybeSingle()),
    loadQuery<any[]>(input.workspaceClient.from("opening_balance_setups").select("id, company_id, income_year, bank_balance, share_capital, share_count, nominal_value, locked_at, created_by").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("ledger_entries").select("id, company_id, setup_id, income_year, entry_type, memo, lines, risk_flags, warning_accepted_by, warning_accepted_at, created_by, created_at").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("holding_actions").select("id, company_id, income_year, action_type, action_date, payload, ledger_entry_id, bank_transaction_id, document_id, risk_level, blocker_code, created_by, created_at").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("bank_transactions").select("id, company_id, income_year, transaction_date, text, amount, balance, source_hash, matched_entry_id, matched_action_id, accepted_warning, created_by, created_at").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("documents").select("id, company_id, income_year, document_type, name, linked_to, status, retention_years, storage_key, created_by, created_at").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("filing_overrides").select("id, preview_id, company_id, income_year, filing, field_target, old_value, new_value, reason, risk_level, owner_confirmed_by, owner_confirmed_at, created_by, created_at").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("period_locks").select("id, company_id, income_year, reason, locked_by, locked_at").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any>(input.workspaceClient.from("annual_data").select("id, company_id, income_year, answers, confirmations, no_activity_confirmed, annual_full_time_equivalents, completed_by, completed_at, updated_by, updated_at").eq("company_id", companyId).eq("income_year", incomeYear).maybeSingle()),
    loadQuery<any>(input.workspaceClient.from("billing_accounts").select("company_id, pricing_plan, monthly_nok, filing_package_nok, founder_cohort_number, subscription_active, filing_package_paid, supported_case, refund_eligible, refund_completed, no_charge_reason, provider_customer_ref, subscription_provider_ref, filing_package_payment_ref, refund_provider_ref").eq("company_id", companyId).maybeSingle()),
    loadQuery<any[]>(input.workspaceClient.from("authority_permissions").select("company_id, obligation, submitter_user_id, confirmed_by, confirmed_at, production_enabled").eq("company_id", companyId)),
    loadQuery<any[]>(input.workspaceClient.from("filing_previews").select("id, company_id, setup_id, income_year, filing, status, issues, preview, hovedskjema_xml, underskjema_xml, source, created_at").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("filing_submissions").select("id, preview_id, company_id, income_year, filing, mode, adapter_mode, payload_hash, idempotency_key, status, calls, receipt_id, feedback_document_ids, feedback_items, receipt_metadata, submitted_payload_ref, submitted_payload, authority_confirmed_at, preview_confirmed_at, created_at, updated_at, submitted_by").eq("company_id", companyId).eq("income_year", incomeYear)),
    loadQuery<any[]>(input.workspaceClient.from("authority_test_runs").select("company_id, obligation, status, receipt_reference, archive_reference, recorded_at").eq("company_id", companyId).eq("obligation", "aksjonaerregisteroppgaven")),
    loadQuery<any>(input.workspaceClient.from("step_up_events").select("actor_id, mfa_verified_at").eq("actor_id", input.actorId).order("mfa_verified_at", { ascending: false }).limit(1).maybeSingle()),
    loadQuery<any>(input.workspaceClient.from("production_security_grants").select("actor_id, security_review_approved, production_credentials_enabled, expires_at, revoked_at").eq("actor_id", input.actorId).maybeSingle()),
    loadQuery<any[]>(input.controlClient.from("launch_signoffs").select("key, status, reviewer, reviewed_at, evidence_link, decision, recorded_by, updated_at").eq("key", "rf1086_authority")),
    loadQuery<any[]>(input.workspaceClient.from("filing_review_comments").select("preview_id, severity").eq("preview_id", preview.id).eq("severity", "hard_block")),
  ]);
