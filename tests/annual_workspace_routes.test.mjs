...
test("dashboard links authenticated owners into the annual workspace", async () => {
  const source = await readFile("app/(owner)/dashboard/page.tsx", "utf8");
  assert.match(source, /annualOverviewHref/);
  assert.match(source, /primaryCompany\.id/);
});
