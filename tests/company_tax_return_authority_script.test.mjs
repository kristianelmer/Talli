...
test("company tax rehearsal is resumable, company-bound, and keeps final submission human-controlled", () => {
  ...
  assert.match(source, /companyPartyNumber:\s*current\.partyNumber/u);
  assert.match(source, /await client\.replaceEnvelope/u);
  assert.match(source, /evidence\.instance\.validationJobId = null/u);
  assert.doesNotMatch(source, /evidence\.(?:partyNumber|companyPartyNumber)/u);
