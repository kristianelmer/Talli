...
test("one effective fact set reconciles tax, annual accounts, SAF-T input, archive, and RF-1086", () => {
  ...
  const saftSourceIds = ledgerEntries
    ...
    .sort();
  assert.deepEqual(saftSourceIds, activity.map((item) => item.id).sort());
  assert.deepEqual(
...
test("every approved golden case has one identical investments, ledger, and SAF-T source identity", () => {
...
test("a correction retains history while every effective output counts only the replacement", () => {
  ...
  assert.equal(archive.investmentCorrections[0].original_action_id, original.id);
});
