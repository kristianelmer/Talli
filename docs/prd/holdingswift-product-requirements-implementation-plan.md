...
| RF-1086 | XSD-backed generator, readiness gate, explicit authority transport plan, and hard-disabled unimplemented production adapter | Implement token/transport journal, retrieve feedback/receipts, and record TT02 evidence and dated approval. |
| Company tax return | Deterministic payload and XML renderer validated against the official 2025 `skattemeldingUpersonlig_v5_ekstern.xsd` and `naeringsspesifikasjon_v6_ekstern.xsd`; reconciliation handles supported admin costs, interest, exempt dividends/gains, non-deductible losses, and 3% inclusion | Implement validation/submission transport, personal signing handoff, feedback/receipt archive, test-environment evidence, and dated approval. |
| Annual accounts | Deterministic result/balance field model, corrected financial income/cost treatment, explicit Altinn 3/ID-porten adapter plan, and fail-closed production gate | Validate the complete payload in Altinn TT02, implement instance/attachment/signing handoff and receipt polling, and record dated approval. |
