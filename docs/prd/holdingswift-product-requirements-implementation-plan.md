...
| Company tax return | Deterministic payload/XML, three official 2025 XSD checks, test-only Skatteetaten/Altinn transport client, TT02 `validertOK` evidence, both Altinn instance scopes, an initialized synthetic 2025 company whose current draft is verified through person and system-user access, and a company/year-bound runtime evidence importer that always remains `pending` without an explicit verified authority outcome; reconciliation handles supported admin costs, interest, exempt dividends/gains, non-deductible losses, and 3% inclusion | Run the separately approved TT02 instance/upload step, complete personal signing handoff and receipt/archive, execute the evidence import in the deployed runtime, integrate final filing state/feedback, and obtain dated approval. |
