...
| Annual accounts | Deterministic result/balance field model including tax expense/payable, live-schema RR0002 XML renderer, test-only Altinn create/upload/validate/lock/person-signing-handoff client, required Altinn instance scopes, live TT02 resource `app_brg_aarsregnskap-vanlig-202406`, accepted synthetic-company system user, zero-error validation, high-assurance person signature/submission, read-only completion polling, official PDF receipt, and archived TT02 instance | Capture the later Regnskapsregisteret processing decision, persist the verified evidence shape in the deployed runtime, and record dated production approval. |
