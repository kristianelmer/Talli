...
| Company tax return | Deterministic payload/XML, three official 2025 XSD checks, test-only Skatteetaten/Altinn transport client, both Altinn instance scopes, initialized current-draft access, current-party-number binding in memory, TT02 file-scan/preflight/asynchronous `validertOK`, high-assurance owner signing, official feedback receipt, archived instance, machine-checked sanitized evidence, and a company/year-bound runtime evidence importer that remains `pending` without an explicit verified authority outcome; reconciliation handles supported admin costs, interest, exempt dividends/gains, non-deductible losses, and 3% inclusion | Execute the evidence import in the deployed runtime, persist structured final authority feedback/state, enforce the no-attachment boundary, complete production credential/security/restore review, and obtain dated `tax_return_authority` approval. |
...
- The supported company-tax TT02 rehearsal completed the current-draft-bound
  ...
  machine-tested and production remains disabled.
