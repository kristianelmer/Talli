...
Evidence baseline: commits through `e8d5a84`
...
- The annual-accounts client is fixed to TT02 and the current
  ...
  bearer material, provider paths, or provider values. The guarded operator
  command advances one operation per invocation, requires explicit test
  execution, and separately gates the lock transition. Provider validation,
  live TT02 evidence, and the personal ID-porten step remain open.
