...
`#188 → #140 → #189 → #141 → #142 → #143 → #190 → #147 → #144 → #145 → #148 → #191 → #137 → #192 → #150 → #151 → #146 → #152 → #153 → #193 → #149 → #194 → #155 → #156 → #157 → #195 → #154`
...
| `P` | public acquisition presentation; no business data owner | Current exact reservation: `apps/web/features/public-acquisition/**`, `apps/web/app/page.tsx`, `apps/web/app/page.module.css`, `apps/web/tests/public-acquisition.test.mjs`, and `tests/launch_copy.test.mjs`. Every other supporting file—including precheck actions/components, legal/help/security/status/SEO routes, root layout/styles and shared components—is integration-owner-only until an exact path reservation is added at a #196 boundary. Company-access owns consent/precheck persistence, billing owns checkout, and measurement storage needs an explicitly declared backend-system technical seam; generated contracts/database/compatibility files are forbidden to `P` |
...
|---|---|---|---|
| #188 | #139 closed; zone `L`. `architecture/compatibility.json` registry-enables banking/#140 as the next capability because its schema has no idle state, but open #188 route-blocks any #140 claim or banking migration; registry activation is not start authority | Full-year 1 January reconstruction, completeness/gap ledger and every #172 supported ledger pattern; `U C D W S X G G2` with filings/SAF-T/archive projections | Block posting on gaps/unsupported cases; reverse rather than mutate; close only with official mappings, golden/cross-output facts and no duplicate rules. Then and only then claim #140. `local` |
...
|---|---|
| #188 | A1→`U D W X`; A2→`U D S`; A3→`U X H`; A4→`U D W`; A5→`X`; A6→`C A`; A7→`D W S G G2` |
