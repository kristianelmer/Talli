"""Explicit owner recovery HTTP boundary with synthetic persistence/provider seams."""
...
from test_annual_refund_recovery_service import ACTOR, COMPANY, PURCHASE, QUERY, REQUEST, RecoveryStore, Reconciler
from test_annual_refund_service import NOW, observation
from talli_backend.application.annual_billing import AnnualRefundRecoveryWorkflow
...
PATH = '/api/v1/billing/annual/refund-recoveries'
HEADERS = {'Authorization': 'Bearer verified-owner', 'X-Request-ID': 'refund-recovery-http'}
BODY = {'companyId': str(COMPANY), 'purchaseId': str(PURCHASE), 'refundRequestId': str(REQUEST)}
...
@pytest.mark.parametrize('field', ['actorId', 'sourceReference', 'idempotencyKey', 'amountMinor', 'incomeYear', 'caseId', 'provider', 'operationId', 'supportCaseId'])
...
@pytest.mark.parametrize('field', ['companyId', 'purchaseId', 'refundRequestId'])
...
@pytest.mark.parametrize('mode', ['owner', 'mfa', 'provider', 'missing'])
def test_denial_and_unavailable_results_do_not_claim_success(mode):
    ...
    assert response.status_code == {'owner': 403, 'mfa': 403, 'provider': 503, 'missing': 404}[mode]
...
def test_workflow_rejects_mismatched_session_actor():
    ...
    with pytest.raises(BillingError):
        AnnualRefundRecoveryWorkflow(sessions, provider)
