"""Frozen RF journal/feedback parity using local journals and provider fakes only."""
...
from talli_backend.compatibility.rf1086_authority_workflow import (
    JournaledRf1086ProductionInput, ProductionOperation, RF1086_FEEDBACK_NAMESPACES,
    Rf1086AuthorityCall, Rf1086AuthorityDocument, Rf1086AuthorityError,
    Rf1086BlockedProductionOperationError, Rf1086Confirmation, Rf1086DocumentPage,
    Rf1086DocumentReference, Rf1086FeedbackArtifactPersistenceError, Rf1086MainResponse,
    Rf1086PostResponse, Rf1086ReconciliationInput, Rf1086ReconciliationSnapshot,
    Rf1086UnknownProductionOutcomeError, classify_rf1086_feedback,
    create_rf1086_feedback_artifact_persistence_error, execute_journaled_rf1086_production,
    execute_rf1086_production_release, reconcile_journaled_rf1086_production,
...
from talli_backend.compatibility.rf1086_authority_workflow import (
    LegacyRf1086Error, Rf1086Approval, Rf1086Connection, Rf1086MutationBinding, Rf1086Preview,
    Rf1086ReadOnlyBinding, Rf1086Submission, reconcile_rf1086_production,
    rf1086_current_manifest, rf1086_current_manifest_hash, rf1086_preview_payload_hash, send_approved_rf1086_production_filing,
)
...
class CoordinatorSession:
    ...
    def event(self, name):
        ...
        if self.failure == name:
            raise LegacyRf1086Error("configuration_unavailable" if name == "configuration" else "status_unavailable")
...
def test_repeated_owner_send_uses_original_journal_and_never_reposts_unknown_mutation():
    ...
    for _ in range(2):
        with pytest.raises(LegacyRf1086Error, match="send_unavailable"):
            send(session)
...
def test_send_rejects_invalid_basis_before_any_token_or_submission(change, code):
    session = CoordinatorSession()
    change(session)
    with pytest.raises(LegacyRf1086Error, match=code):
        send(session)
...
def test_js_json_and_immutable_approval_order_match_original_source_across_character_sets():
    ...
    import subprocess
    from talli_backend.compatibility.rf1086_authority_workflow import _json, rf1086_production_document_order
    cases = [
        {"b": "<B/>", "a": "<A/>", "A": "<Upper/>"},
        {"é": "<Accent/>", "e\u0301": "<Decomposed/>", "ø": "<Norwegian/>", "Å": "<Ring/>"},
        {"😀": "<Astral/>", "\ue000": "<Private/>", "a": "<A/>"},
        {"20": "<Twenty/>", "3": "<Three/>", "01": "<Leading/>", "4294967295": "<Max/>", "0": "<Zero/>"},
        {"a ": "<Trailing/>", " a": "<Leading/>", "\ufeffa\ufeff": "<Bom/>"},
        {DOCUMENT: "<B/>", MAIN: SUB_XML},
    ]
    import random
    generator = random.Random(150)
    cases.append({str(UUID(int=generator.getrandbits(128))): "<U/>" for _ in range(128)})
    previews = [Rf1086Preview(PREVIEW, COMPANY, 2025, "aksjonaerregisteroppgaven", MAIN_XML, value,
        ("\ufeff warning \ufeff", "😀", "\ue000", "\x85retained", " warning ", "\u2003")) for value in cases]
    script = '''import {createHash} from 'node:crypto';
import {buildProductionApprovalManifest,productionApprovalHash} from './apps/web/app/lib/production-approval.ts';
let raw=''; for await(const chunk of process.stdin) raw+=chunk;
const sha=value=>createHash('sha256').update(value,'utf8').digest('hex');
console.log(JSON.stringify(JSON.parse(raw).map(v=>{
const payload={filing:v.filing,company_id:v.company_id,income_year:v.income_year,hovedskjema_xml:v.hovedskjema_xml,underskjema_xml:v.underskjema_xml};
const manifest=buildProductionApprovalManifest({companyId:v.company_id,userId:v.actor_id,organizationNumber:'310279617',incomeYear:v.income_year,obligation:'aksjonaerregisteroppgaven',caseProfile:'rf1086_no_activity_v1',adapterVersion:'rf1086-production-v1',previewId:v.id,payloadHash:sha(JSON.stringify(payload)),documentHashes:{hovedskjema:sha(v.hovedskjema_xml),...Object.fromEntries(Object.entries(v.underskjema_xml).map(([key,xml])=>['underskjema_'+key,sha(xml)]))},blockers:[],warnings:v.warnings});
return {manifest,hash:productionApprovalHash(manifest),payloadHash:sha(JSON.stringify(payload)),order:Object.keys(v.underskjema_xml).sort((a,b)=>a.localeCompare(b))};
})));'''
    from dataclasses import asdict
    result = subprocess.run(["node", "--input-type=module", "-e", script],
        input=json.dumps([asdict(preview) | {"actor_id": OWNER} for preview in previews]),
        text=True, capture_output=True, check=True, timeout=15, cwd=Path(__file__).resolve().parents[3])
...
def test_current_payload_cannot_reuse_a_changed_or_incomplete_approval_manifest(change):
    import copy
    session = CoordinatorSession()
    manifest = copy.deepcopy(session.approval.manifest)
