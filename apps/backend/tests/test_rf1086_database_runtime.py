...
from talli_backend.compatibility.rf1086_authority_workflow import (
    LegacyRf1086Error, ProductionOperationFailure, Rf1086Preview, Rf1086ReconciliationArtifact,
    Rf1086ReconciliationSnapshot, rf1086_current_manifest, rf1086_current_manifest_hash, rf1086_preview_payload_hash,
)
...
def backend_url():
    ...
    with psycopg.connect(DATABASE_URL) as connection:
        ...
        connection.execute(sql.SQL("grant legacy_rf1086_executor,documents_executor to {} with inherit false,set true").format(sql.Identifier(username)))
    try:
        yield make_conninfo(DATABASE_URL, user=username, password=password)
    finally:
        with psycopg.connect(DATABASE_URL) as connection:
            connection.execute(sql.SQL("revoke legacy_rf1086_executor,documents_executor from {}").format(sql.Identifier(username)))
...
def fixture(backend_url):
    ...
    with psycopg.connect(DATABASE_URL) as connection:
        ...
        insert(connection, "public.filing_previews", {"id": preview_id, "company_id": company, "income_year": 2025,
        ...
        insert(connection, "public.filing_approval_snapshots", {"id": approval, "entitlement_id": entitlement, "preview_id": preview_id,
        ...
        insert(connection, "public.authority_permissions", {"company_id": company, "obligation": OBLIGATION,
    ...
    try:
        ...
        with psycopg.connect(DATABASE_URL) as connection:
            connection.execute("delete from public.production_feedback_artifacts where company_id=%s", (company,))
            ...
            connection.execute("delete from public.production_filing_submissions where company_id=%s", (company,))
            connection.execute("delete from public.filing_approval_snapshots where company_id=%s", (company,))
            connection.execute("delete from public.filing_previews where company_id=%s", (company,))
...
def store(fixture, *, actor=None, fresh=True, documents=None, storage_transport=None):
    ...
    result = PostgresLegacyRf1086AuthoritySession(LedgerSupabaseConfiguration("https://project.example.test", "", fixture["url"]),
...
def test_contracted_begin_retains_exact_bound_identity_and_needs_no_foreign_table_grants(fixture):
    ...
    with psycopg.connect(DATABASE_URL) as connection:
        ...
        assert connection.execute("select pg_has_role('talli_ledger_backend','legacy_rf1086_executor','SET'), "
            "pg_has_role('talli_ledger_backend','legacy_rf1086_executor','USAGE')").fetchone() == (True, False)
        assert connection.execute("select company_id,user_id,approval_id,entitlement_id,status from public.production_filing_submissions where id=%s", (submission,)).fetchone() == (
        ...
        for table in ("billing.production_pilot_entitlements", "authority_connections.system_user_requests", "public.documents"):
            assert not connection.execute("select has_table_privilege('legacy_rf1086_executor',%s,'SELECT')", (table,)).fetchone()[0]
        assert not connection.execute("select has_table_privilege('legacy_rf1086_executor','public.production_filing_events','INSERT')").fetchone()[0]
...
def test_begin_rechecks_each_original_release_gate_without_creating_submission(fixture, gate):
    with psycopg.connect(DATABASE_URL) as connection:
        if gate == "approval_invalidated":
            connection.execute("update public.filing_approval_snapshots set invalidated_at=now(),invalidation_reason='fixture' where id=%s", (fixture["approval"],))
            ...
            connection.execute("update public.authority_permissions set production_enabled=false where company_id=%s", (fixture["company"],))
    ...
    with pytest.raises((LegacyRf1086Error, _PersistenceError)):
        begin(fixture, store(fixture, fresh=gate != "stale_mfa"))
    with psycopg.connect(DATABASE_URL) as connection:
        assert connection.execute("select count(*) from public.production_filing_submissions where company_id=%s", (fixture["company"],)).fetchone()[0] == 0
    ...
    with psycopg.connect(fixture["url"]) as connection:
        ...
        with pytest.raises(psycopg.errors.RaiseException, match="verified_owner_required"):
            with connection.transaction():
                connection.execute("select * from public.filing_approval_snapshots")
    with psycopg.connect(DATABASE_URL) as connection:
...
def test_interrupted_preparation_preserves_uuid_body_and_never_becomes_fresh_preparation(fixture):
    ...
    with psycopg.connect(DATABASE_URL) as connection:
        assert connection.execute("select count(*) from public.production_filing_events where submission_id=%s", (submission,)).fetchone()[0] == 1
...
def test_recovery_lease_uses_confirmed_reference_after_entitlement_expiry_and_stale_mfa(fixture):
    ...
    with pytest.raises(LegacyRf1086Error):
...
def test_real_documents_contract_stored_receipt_and_hash_drive_final_feedback_without_metadata_duplication(fixture):
    ...
    with psycopg.connect(DATABASE_URL) as connection:
        row = connection.execute("select d.status,d.content_sha256,d.byte_length,d.storage_key,a.sha256 "
            "from public.production_feedback_artifacts a join public.documents d on d.id=a.document_id where a.submission_id=%s", (submission,)).fetchone()
        assert row[:3] == ("stored", digest, len(raw)) and row[4] == digest
        assert row[3].startswith(str(fixture["company"]) + "/2025/")
        assert connection.execute("select count(*) from public.production_feedback_artifacts where submission_id=%s", (submission,)).fetchone()[0] == 1
        ...
        with psycopg.connect(DATABASE_URL) as lock:
            ...
            for _ in range(60):
                ...
                with psycopg.connect(DATABASE_URL) as observer:
                    observed = observer.execute("select exists(select 1 from pg_stat_activity where wait_event_type='Lock' and query like 'select id from public.begin_production_filing%')").fetchone()[0]
                    if observed:
