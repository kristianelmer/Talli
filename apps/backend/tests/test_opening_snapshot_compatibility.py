...
def opening_view() -> LegacyOpeningSnapshotView:
    return LegacyOpeningSnapshotView(
        setup_id=SETUP_ID,
...
def test_legacy_opening_pages_own_a_bounded_opaque_cursor() -> None:
    cursor = LegacyOpeningSnapshotCursor("opaque-next")
    page = LegacyOpeningSnapshotPage(
        items=(opening_view(),),
        next_cursor=cursor,
        has_more=True,
    )
    ...
    with pytest.raises(ValueError):
        LegacyOpeningSnapshotCursor("x" * 4097)
    with pytest.raises(ValueError):
        LegacyOpeningSnapshotPage(
            items=(opening_view(),),
            next_cursor=None,
