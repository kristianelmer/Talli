"""Accounts-owned normalization of the existing filing controls."""
from dataclasses import replace

from .public import AnnualAccountsError
...
def normalize_override(command):
    ...
    if result.owner_confirmed is not True:
        raise AnnualAccountsError.invalid_input('Overstyring må bekreftes av eier')
    if not result.field_target:
        raise AnnualAccountsError.invalid_input('Feltmål mangler for filing-overstyring.')
    if not (result.old_value or result.new_value):
        raise AnnualAccountsError.invalid_input('Gammel eller ny verdi må fylles ut for filing-overstyring.')
    if not result.reason:
        raise AnnualAccountsError.invalid_input('Begrunnelse mangler for filing-overstyring.')
    if result.risk_level not in ('advisory', 'warning', 'block'):
        raise AnnualAccountsError.invalid_input('Ugyldig risikonivå for filing-overstyring.')
    return result
...
def normalize_review(command):
    ...
    if result.severity not in ('advisory', 'hard_block'):
        raise AnnualAccountsError.invalid_input('Ugyldig kommentaralvorlighet')
    if not result.body:
        raise AnnualAccountsError.invalid_input('Kommentar mangler')
    return result


def normalize_test_evidence(command):
    if command.environment not in ('test', 'manual_evidence'):
        raise AnnualAccountsError.invalid_input('Ugyldig testmiljø')
    if command.status not in ('accepted', 'rejected', 'blocked', 'pending'):
        raise AnnualAccountsError.invalid_input('Ugyldig teststatus.')
    ...
    if not result.test_reference:
        raise AnnualAccountsError.invalid_input('Testreferanse mangler.')
    return result
