...
from .public import CompanyTaxError, CompanyTaxFilingRows


def validate_rows(value: CompanyTaxFilingRows) -> None:
    try:
        ...
        for name in ('previews', 'submissions', 'overrides', 'review_comments', 'permissions', 'test_evidence'):
            ...
            for row in rows:
                ...
                if name in ('permissions', 'test_evidence'):
                    if row['obligation'] != 'skattemelding':
                        raise ValueError()
                elif name == 'review_comments':
                    if row['preview_id'] not in previews:
                        raise ValueError()
                else:
                    if (row['filing'] != 'skattemelding for AS'
