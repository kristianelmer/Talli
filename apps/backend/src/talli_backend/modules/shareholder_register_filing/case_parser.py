...
# Private admission preserves the predecessor Pydantic input/coercion contract.
# Public immutable case values never expose these parsing models.
def parse_rf1086_case(value: object):
    from . import public
    parsed = FilingCase.model_validate_json(value) if isinstance(value, (str, bytes, bytearray)) else FilingCase.model_validate(value)
    def freeze(model):
        if isinstance(model, BaseModel):
            target_name = "Rf1086Case" if isinstance(model, FilingCase) else "Rf1086" + type(model).__name__
            return getattr(public, target_name)(**{name: freeze(getattr(model, name)) for name in type(model).model_fields})
        if isinstance(model, ShareholderKind):
            return public.Rf1086ShareholderKind(model.value)
        if isinstance(model, list):
            return tuple(freeze(item) for item in model)
        return model
