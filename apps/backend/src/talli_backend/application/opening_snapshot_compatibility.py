"""Fail-closed read models for the frozen opening-snapshot compatibility store.

These types are deliberately outside every capability public package. Stage #139
may retire the ledger legacy facade, but ADR-0013 keeps the future shareholder-
register filing public contract and business behavior frozen until its own stage.
"""
...
class LegacyOpeningSnapshotCursor:
    value: str
...
@dataclass(frozen=True, slots=True)
class LegacyOpeningShareholderView:
    shareholder_id: str
    setup_id: str
...
@dataclass(frozen=True, slots=True)
class LegacyOpeningSnapshotView:
    setup_id: str
    company_id: CompanyId
    ...
    created_by: ActorId
    shareholders: tuple[LegacyOpeningShareholderView, ...]
...
@dataclass(frozen=True, slots=True)
class LegacyOpeningSnapshotPage:
    items: tuple[LegacyOpeningSnapshotView, ...]
    next_cursor: LegacyOpeningSnapshotCursor | None
    has_more: bool
