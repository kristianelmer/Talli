...
const identity = { initiatingUserId: uid(1), companyId: uid(2), purchaseId: uid(3), supportCaseId: uid(5) };
const cursors = { beforePurchaseId: uid(6) };
...
const actionSource = actions.slice(actions.indexOf("export async function recoverAnnualSupportCleanup("), actions.indexOf("export async function recoverAnnualSupportRefund("));
function action({ token = "verified-operator", userId = identity.initiatingUserId, userError = null,
  ...
  return { calls, refreshes, verifiedTokens, run: compile(actionSource, {}, {
    ...
    recoverAnnualSupportCleanupThroughApi: async (...args) => {
...
const expectedBody = { companyId: identity.companyId, purchaseId: identity.purchaseId,
  supportCaseId: identity.supportCaseId };
...
test("explicit repeated operator checks retain the same three IDs and no attempt key", async () => {
  const harness = action();
...
const controlSource = readFileSync(new URL("../app/components/billing/AnnualSupportCleanupRecoveryControl.tsx", import.meta.url), "utf8");
function control(recoverAction) {
  ...
  }, { FormData: class extends FormData { constructor(value) { super(); if (value) for (const [key, item] of value) this.append(key, item); } } }).AnnualSupportCleanupRecoveryControl;
...
test("actual control stays read-only on render and blocks duplicate events until same-request pending recovery settles", async () => {
  ...
  assert.equal(calls.length, 0); assert.match(idle, /Sjekk avtalestopp/);
  ...
  assert.match(harness.render(), /Avtalestoppet er ikke bekreftet/);
...
test("lost action response leaves original scope for explicit retry and never posts during a fresh mount", async () => {
  ...
  assert.equal(destination.searchParams.get("supportCase"), identity.supportCaseId);
...
test("feedback remains operation-scoped and mismatched identity never announces confirmation", async () => {
  for (const field of Object.keys(identity)) {
    ...
    harness.render(); await harness.submit();
    assert.doesNotMatch(harness.render(), /Betalingsleverandøren har bekreftet avtalestoppet/);
  }
  const harness = control(async () => ({ kind: "observed", ...identity, status: "confirmed" }));
  harness.render(); await harness.submit();
  assert.match(harness.render(), /Betalingsleverandøren har bekreftet avtalestoppet/);
});
