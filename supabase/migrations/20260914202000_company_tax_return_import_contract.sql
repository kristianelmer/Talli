-- #152 owned TT02 persistence and canonical legacy Audit inclusion.
-- Both contracts share the application transaction; expanded copies cannot write.
begin;
set local lock_timeout='5s';
set local statement_timeout='120s';
set local search_path='';
do $audit_role$ begin
 if not exists(select 1 from pg_catalog.pg_roles where rolname='audit_legacy_append_owner') then
  create role audit_legacy_append_owner nologin noinherit nobypassrls;
 end if;
 if exists(select 1 from pg_catalog.pg_roles where rolname='audit_legacy_append_owner'
   and (rolcanlogin or rolinherit or rolbypassrls or rolsuper)) then
  raise exception 'audit_legacy_append_role_is_not_restricted';
 end if;
end; $audit_role$;
create temporary table tax152_import_borrowed_roles(role_name name primary key,prior jsonb) on commit drop;
do $borrow$ declare r text; v_prior jsonb; begin
 for r in select unnest(array['company_tax_filing_store_owner','company_access_executor','audit_legacy_append_owner']) loop
 if not pg_catalog.pg_has_role(current_user,r,'SET') then
  ...
  insert into tax152_import_borrowed_roles values(r,v_prior);
  execute pg_catalog.format('grant %I to %I with set true granted by %I',r,current_user,current_user);
 end if;
end loop;
end; $borrow$;

select pg_catalog.set_config('talli.tax152.audit_public_create',pg_catalog.has_schema_privilege('audit_legacy_append_owner','public','CREATE')::text,true);
grant usage,create on schema public to audit_legacy_append_owner;
grant insert on public.audit_events to audit_legacy_append_owner;
grant execute on function public.company_access_auth_uid_v1(),public.company_access_auth_jwt_v1(),public.company_access_is_accepted_owner_v1(uuid) to audit_legacy_append_owner;
set local role company_access_executor;
grant execute on function public.company_access_read_rf_company_identity_v1(uuid,text) to company_tax_filing_store_owner,company_tax_filing_workflow_executor;
reset role;
-- The legacy Audit table and original producer policies remain canonical.
create policy tax_tt02_legacy_audit_append on public.audit_events for insert to audit_legacy_append_owner
with check(actor_id=public.company_access_auth_uid_v1() and public.company_access_is_accepted_owner_v1(company_id));
set local role audit_legacy_append_owner;
create function public.audit_include_filing_event_v1(p_company uuid,p_actor text,p_category text,p_action text,p_message text) returns void
...
begin
 if a is null or a::text is distinct from p_actor
  or pg_catalog.current_setting('talli.verified_actor_id',true) is distinct from a::text
  or claims->>'sub' is distinct from a::text or claims->>'role' is distinct from 'authenticated'
  or public.company_access_auth_jwt_v1()->>'aal' is distinct from 'aal2'
  or not public.company_access_is_accepted_owner_v1(p_company) then
  raise exception 'company_tax_return_forbidden';
 end if;
 if p_category is distinct from 'submission' or p_action is distinct from 'company_tax_tt02_evidence_imported'
  or p_message is null or p_message !~ '^Skattemelding TT02-evidens importert og venter på klassifisering med ref tt02:[0-9]+/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}[.]$' then
  raise exception 'company_tax_return_invalid_input';
 end if;
 insert into public.audit_events(company_id,actor_id,category,action,message)
 values(p_company,a,p_category,p_action,p_message);
end; $function$;
revoke all on function public.audit_include_filing_event_v1(uuid,text,text,text,text) from public,anon,authenticated,service_role;
grant execute on function public.audit_include_filing_event_v1(uuid,text,text,text,text) to company_tax_filing_workflow_executor;
reset role;
do $restore_create$ begin
 if not pg_catalog.current_setting('talli.tax152.audit_public_create')::boolean then
  revoke create on schema public from audit_legacy_append_owner;
 end if;
end; $restore_create$;
set local role company_tax_filing_store_owner;
create policy tax_return_owner_insert on company_tax_filing.authority_test_runs for insert to company_tax_filing_store_owner
with check(company_tax_filing.return_phase_v1() in ('cutover','contracted') and public.company_access_is_accepted_owner_v1(company_id) and recorded_by=public.company_access_auth_uid_v1());
create policy tax_return_owner_lock on company_tax_filing.authority_test_runs for update to company_tax_filing_store_owner
...
create policy tax_return_owner_insert on company_tax_filing.filing_submissions for insert to company_tax_filing_store_owner
with check(company_tax_filing.return_phase_v1() in ('cutover','contracted') and public.company_access_is_accepted_owner_v1(company_id) and created_by=public.company_access_auth_uid_v1());
create policy tax_return_owner_lock on company_tax_filing.filing_submissions for update to company_tax_filing_store_owner
using(company_tax_filing.return_phase_v1() in ('cutover','contracted') and public.company_access_is_accepted_owner_v1(company_id))
with check(company_tax_filing.return_phase_v1() in ('cutover','contracted') and public.company_access_is_accepted_owner_v1(company_id));
create function company_tax_filing.import_tt02_evidence_v1(p_payload jsonb,p_subject text)
returns jsonb
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_actor_id uuid := company_tax_filing.verified_filing_actor_v1(p_subject);
  v_authority_payload jsonb;
  v_submission_payload jsonb;
  v_authority company_tax_filing.authority_test_runs%rowtype;
  v_submission company_tax_filing.filing_submissions%rowtype;
  v_persisted_authority company_tax_filing.authority_test_runs%rowtype;
  v_persisted_submission company_tax_filing.filing_submissions%rowtype;
...
begin
  ...
  if (select public.company_access_auth_jwt_v1() ->> 'aal') is distinct from 'aal2' then
  ...
  begin
    ...
    from jsonb_populate_record(null::company_tax_filing.authority_test_runs, v_authority_payload);
    select * into v_submission
    from jsonb_populate_record(null::company_tax_filing.filing_submissions, v_submission_payload);
  exception
    when others then
      raise exception 'company_tax_evidence_invalid_payload';
  end;
  ...
  if not public.company_access_is_accepted_owner_v1(v_authority.company_id) then
  ...
  begin
    ...
    v_company_org_number := public.company_access_read_rf_company_identity_v1(v_authority.company_id,p_subject)->>'org_number';
  ...
  from company_tax_filing.authority_test_runs r
  ...
  if not found then
    insert into company_tax_filing.authority_test_runs (
      company_id,
      obligation,
      environment,
      status,
  ...
  from company_tax_filing.filing_submissions s
  where s.authority_test_run_id = v_persisted_authority.id
  for update;

  if not found then
    insert into company_tax_filing.filing_submissions (
      preview_id,
...
revoke all on function company_tax_filing.import_tt02_evidence_v1(jsonb,text) from public,anon,authenticated,service_role;
grant execute on function company_tax_filing.import_tt02_evidence_v1(jsonb,text) to company_tax_filing_workflow_executor;
reset role;
do $restore$ declare r record; begin
 for r in select * from tax152_import_borrowed_roles loop
