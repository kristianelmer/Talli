...
-- Supabase's migration role can use Auth helpers but cannot delegate USAGE on
-- the auth schema. Keep executor roles out of that schema and expose only the
-- two identity values this capability needs through migration-owned definers.
...
as $function$
  ...
  from auth.users u
  where u.id = (select public.company_access_auth_uid_v1())
...
as $function$
  select exists (
    ...
    where r.operation_id = p_operation_id
      and r.actor_id = (select public.company_access_auth_uid_v1())
...
using (
  ...
  or exists (
    ...
    where i.company_id = companies.id
      and i.invited_email = lower(coalesce((select public.company_access_auth_jwt_v1()) ->> 'email', ''))
  )
);
...
using (
  ...
  or invited_email = lower(coalesce((select public.company_access_auth_jwt_v1()) ->> 'email', ''))
);
...
to company_access_executor
with check (
  invited_by = (select public.company_access_auth_uid_v1())
...
using (
  (
    coalesce((select public.company_access_auth_jwt_v1()) ->> 'aal', '') = 'aal2'
    and public.company_access_is_accepted_owner_v1(company_id)
  )
  or invited_email = lower(coalesce((select public.company_access_auth_jwt_v1()) ->> 'email', ''))
)
with check (
  ...
  and (
    ...
    or (
      invited_email = lower(coalesce((select public.company_access_auth_jwt_v1()) ->> 'email', ''))
      and accepted_by = (select public.company_access_auth_uid_v1())
      and status = 'accepted'
    )
  )
);
...
using (
  (
    ...
    and actor_id = (select public.company_access_auth_uid_v1())
  )
  or (
    command_name in ('create_invitation', 'revoke_invitation', 'resend_invitation', 'administer_membership')
    and actor_id = (select public.company_access_auth_uid_v1())
...
on public.company_memberships for select
to company_access_recovery_executor
using (user_id = (select public.company_access_auth_uid_v1()) and accepted_at is not null);
...
with check (
  ...
  and exists (
    select 1 from public.company_access_command_receipts r
    where r.actor_id = (select public.company_access_auth_uid_v1())
...
to company_access_executor
with check (actor_id = (select public.company_access_auth_uid_v1()));
...
begin
  ...
  if v_current_subject is null
     ...
     or v_current_email <> pg_catalog.lower(coalesce(public.company_access_auth_jwt_v1() ->> 'email', '')) then
    return;
  end if;
  return query select
...
declare
  ...
  v_now timestamptz := statement_timestamp();
begin
  if v_actor_id is null or coalesce(public.company_access_auth_jwt_v1() ->> 'aal', '') <> 'aal2'
     or not public.company_access_is_accepted_owner_v1(p_company_id) then
    raise exception 'company_access_not_found' using errcode = 'P0001';
  end if;
  perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(v_actor_id::text || '|' || p_operation_id::text, 160));
...
begin
  ...
  if p_user_id = v_actor_id or p_expected_role not in ('reviewer', 'read_only')
  ...
  insert into public.company_access_command_receipts (
    operation_id, command_name, actor_id, company_id, request_fingerprint, result,
    side_effects_completed_at, expires_at
  ) values (
...
create or replace function public.company_access_side_effect_id_v1(
  p_actor_id uuid,
  p_operation_id uuid,
  p_purpose text
)
...
declare
  ...
  v_digest bytea;
  v_hex text;
  v_framed text := '["talli:company-access-side-effect:v1","'
    || p_actor_id::text || '","' || p_operation_id::text || '","' || p_purpose || '"]';
begin
  select n.nspname into v_extension_schema
  from pg_catalog.pg_extension e
  join pg_catalog.pg_namespace n on n.oid = e.extnamespace
  ...
  execute pg_catalog.format(
    ...
    v_extension_schema
  ) into v_digest using v_framed;
  v_digest := pg_catalog.set_byte(v_digest, 6, (pg_catalog.get_byte(v_digest, 6) & 15) | 128);
  v_digest := pg_catalog.set_byte(v_digest, 8, (pg_catalog.get_byte(v_digest, 8) & 63) | 128);
  v_hex := pg_catalog.encode(pg_catalog.substr(v_digest, 1, 16), 'hex');
  return (pg_catalog.substr(v_hex, 1, 8) || '-' ||
    pg_catalog.substr(v_hex, 9, 4) || '-' ||
    pg_catalog.substr(v_hex, 13, 4) || '-' ||
    pg_catalog.substr(v_hex, 17, 4) || '-' ||
    pg_catalog.substr(v_hex, 21, 12))::uuid;
end;
$function$;

create or replace function public.company_access_pending_invitation_side_effects()
returns table (
  operation_id uuid,
  command_name text,
  company_id uuid,
...
begin
  ...
  return query
  select r.operation_id, r.command_name, r.company_id, r.result,
    case when r.expires_at > statement_timestamp() then r.delivery_token else null end
  from public.company_access_command_receipts r
  where r.actor_id = v_actor_id
    and r.command_name in ('create_invitation', 'accept_invitation', 'revoke_invitation', 'resend_invitation')
    and r.side_effects_completed_at is null
    and (
      (r.command_name = 'accept_invitation' and exists (
        select 1 from public.company_memberships m
        where m.company_id = r.company_id and m.user_id = v_actor_id and m.accepted_at is not null
      ))
      or (
        r.command_name <> 'accept_invitation'
        and coalesce(public.company_access_auth_jwt_v1() ->> 'aal', '') = 'aal2'
        and public.company_access_is_accepted_owner_v1(r.company_id)
      )
    )
...
create or replace function public.company_access_complete_invitation_side_effect(
  p_operation_id uuid
)
returns boolean
language plpgsql
...
begin
  ...
  if v_receipt.command_name = 'accept_invitation' then
    if not exists (
      select 1 from public.company_memberships m
      where m.company_id = v_receipt.company_id and m.user_id = v_actor_id and m.accepted_at is not null
    ) then raise exception 'company_access_not_found' using errcode = 'P0001'; end if;
  elsif coalesce(public.company_access_auth_jwt_v1() ->> 'aal', '') <> 'aal2'
  ...
  v_audit_action := case v_receipt.command_name
    when 'create_invitation' then 'reviewer_invitation_created'
    when 'accept_invitation' then 'reviewer_invitation_accepted'
    when 'revoke_invitation' then 'reviewer_invitation_revoked'
    when 'resend_invitation' then 'reviewer_invitation_resent'
    else null end;
  v_audit_message := case v_receipt.command_name
    when 'create_invitation' then 'Reviewer/read-only invitasjon køet for ' || v_role || '.'
    when 'accept_invitation' then 'Invitasjon akseptert som ' || v_role || '.'
    when 'revoke_invitation' then 'Reviewer/read-only invitasjon tilbakekalt.'
    when 'resend_invitation' then 'Reviewer/read-only invitasjon sendt på nytt.'
    else null end || ' Forespørsels-ID: ' || p_operation_id::text || '.';
  if v_audit_action is null then
    raise exception 'company_access_invalid_request' using errcode = 'P0001';
  end if;
  v_audit_id := public.company_access_side_effect_id_v1(
    v_actor_id, p_operation_id, v_receipt.command_name || ':audit'
  );
  if not exists (
    select 1 from public.audit_events a
    where a.id = v_audit_id and a.company_id = v_receipt.company_id
      and a.actor_id = v_actor_id and a.category = 'review'
...
grant usage on schema public to company_access_executor;
...
grant usage on schema public to company_access_recovery_executor;
...
-- PostgreSQL requires the current migration role to be a member of a function's
-- new owner. Keep that membership transaction-local to this atomic block: a
-- failed transfer rolls the grant back, and a successful transfer revokes it.
do $ownership$
begin
  execute pg_catalog.format(
  ...
  revoke create on schema public
    from company_access_executor, company_access_recovery_executor;
  execute pg_catalog.format(
...
revoke all on function public.company_access_auth_uid_v1() from public, anon;
revoke all on function public.company_access_auth_jwt_v1() from public, anon;
...
grant execute on function public.company_access_auth_uid_v1() to authenticated;
grant execute on function public.company_access_auth_jwt_v1() to authenticated;
