-- #152 positive company/year source coverage. Installed after filing expansion;
-- execution remains unavailable before cutover and after rollback.
begin;
set local lock_timeout='5s';
set local statement_timeout='120s';
set local search_path='';
create temporary table tax152_source_borrowed_roles(role_name name primary key,prior jsonb) on commit drop;
do $borrow$ declare r text:='company_tax_filing_store_owner'; v_prior jsonb; begin
 if not pg_catalog.pg_has_role(current_user,r,'SET') then
  ...
  insert into tax152_source_borrowed_roles values(r,v_prior);
...
create function company_tax_filing.read_source_snapshot_v1(p_company uuid,p_year integer,p_subject text) returns jsonb
...
begin
 ...
 if not public.company_access_is_accepted_owner_v1(p_company) then raise exception 'company_tax_return_not_found'; end if;
 if p_year is null or p_year not between 2000 and 2100 then raise exception 'company_tax_return_invalid_input'; end if;
 ...
 phase:=company_tax_filing.return_phase_v1();
 if phase is null or phase not in ('cutover','contracted') then raise exception 'company_tax_return_unavailable'; end if;
 workspace:=company_tax_filing.read_workspace_v1(p_company,p_year,p_subject);
 select s.source_revision into source_revision from backend_system.company_tax_return_migration_state s where singleton;
 ...
 into inventory,valid_inventory from backend_system.company_tax_return_migration_inventory;
 select coalesce(array_agg(distinct family order by family),'{}'::text[]) into reconciled
 from backend_system.company_tax_return_reconciliations r
 ...
 from (select distinct source_id from backend_system.company_tax_return_source_rows
       where family='filing_submissions' and classification='tax'
 ...
 -- Source coverage is bound to the declared seven-table Tax extent. An added
 -- journal/store needs a new source contract before history can be complete.
 select count(*)=7 and pg_catalog.bool_and(c.relname=any(families||array['settlements'])) into extent_valid
 from pg_catalog.pg_class c join pg_catalog.pg_namespace n on n.oid=c.relnamespace
 where n.nspname='company_tax_filing' and c.relkind in ('r','p');
 ...
 select count(*)=5 and pg_catalog.bool_and(x.convalidated and pg_catalog.pg_get_constraintdef(x.oid)=case c.relname
  when 'filing_overrides' then 'CHECK (((filing <> ''skattemelding for AS''::text) AND (field_target !~~ ''skattemelding.%''::text)))'
  when 'authority_permissions' then 'CHECK ((obligation <> ''skattemelding''::text))'
  when 'authority_test_runs' then 'CHECK ((obligation <> ''skattemelding''::text))'
  else 'CHECK ((filing <> ''skattemelding for AS''::text))' end) into fences_valid
 from pg_catalog.pg_constraint x join pg_catalog.pg_class c on c.oid=x.conrelid
 join pg_catalog.pg_namespace n on n.oid=c.relnamespace
 where n.nspname='public' and c.relname in ('filing_previews','filing_submissions','filing_overrides','authority_permissions','authority_test_runs')
  and x.conname='tax152_legacy_writer_retired' and x.contype='c';
 ...
 join backend_system.company_tax_return_migration_inventory m on m.resource='table:public.filing_submissions'
 cross join lateral pg_catalog.jsonb_array_elements(m.definition->'constraints') original
 where x.conrelid='company_tax_filing.filing_submissions'::regclass
 ...
 coverage:=pg_catalog.jsonb_build_object(
  'scope','talli_recorded_company_tax','companyId',p_company,'incomeYear',p_year,
  ...
  'quarantineClear',not exists(select 1 from backend_system.company_tax_return_quarantine),
  'sourceRowsValid',not exists(select 1 from backend_system.company_tax_return_source_rows
...
revoke all on function company_tax_filing.read_source_snapshot_v1(uuid,integer,text) from public,anon,authenticated,service_role;
grant execute on function company_tax_filing.read_source_snapshot_v1(uuid,integer,text) to company_tax_filing_workflow_executor;
reset role;
do $restore$ declare r record; begin
 for r in select * from tax152_source_borrowed_roles loop
