-- Roll back application usage first. Preserve every original source and head.
...
revoke execute on function shareholder_register_filing.lock_year_source_v1(uuid,integer),shareholder_register_filing.append_year_source_v1(uuid,uuid,integer,integer,text,text,text,uuid,text,text,text,timestamptz) from shareholder_register_filing_executor;
revoke select on shareholder_register_filing.year_source_versions,shareholder_register_filing.year_source_heads from shareholder_register_filing_executor;
reset role;
do $restore$
declare r record;
begin
